...
function printwalkprogress(table){
    ...
    let dist = getiwddistance(gameState.getCurrentPosition().statID, gameState.getCurrentPosition().goalStatID);
    ...
    updatetrackprogress(2, timeelapsed/mstime, gameState.getCurrentPosition().goalStatID, gameState.getCurrentPosition().statID);
...
function printcurrentsection(force = false){
    ...
    if (currentsection == 0){
        ...
        if (gameState.getCurrentPosition().transporttype == 1){
            ...
            printschedule(_traintimetable, gameState.getCurrentPosition(), true, true);
        }
    ...
    if (currentsection == 5){
        printwalkable(gameState.getCurrentPosition().transporttype == 2 ? -1 : gameState.getCurrentPosition().statID);
    }

    gameState.updateCurrentPosition({iswifi: false});
    if (gameState.getCurrentPosition().transporttype == 0){
        gameState.updateCurrentPosition({iswifi: havewifistation(gameState.getCurrentPosition().statID, gameState.getCurrentPosition().day)});
    }
    if (gameState.getCurrentPosition().transporttype == 1){
        gameState.updateCurrentPosition({iswifi: havewifi(gameState.getCurrentPosition().lineID, gameState.getCurrentPosition().tripID, gameState.getCurrentPosition().day, timetable.lines[gameState.getCurrentPosition().lineID].type)});
    }
    if (gameState.getCurrentPosition().transporttype == 2){
        ...
        if (Math.abs(current - end) < 60000){
            gameState.updateCurrentPosition({iswifi: havewifistation(gameState.getCurrentPosition().goalStatID, gameState.getCurrentPosition().day)});
        }
    }
    if (gameState.getCurrentPosition().iswifi){
