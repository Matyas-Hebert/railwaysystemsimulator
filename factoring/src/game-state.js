...
export class GameState {
    ...
    #maxEnergy = 5000;
    #energySnapshot = {
        positionAtIdleStart: null, 
        energyRate: constants.ENERGY_RESTORATION_IDLE,
        lastEnergy: 5000,
        idleStartTime: null,
        savedAt: Date.now()
    }
    ...
    #usesRemaining = [];
    #selectedOperator = 0;
    #openedOperatorSelection = false;
    #operatorCount;

    constructor(stations, stationIdByLonLat, lines = [], operatorCount = 0) {
        if (!Number.isInteger(operatorCount) || operatorCount < 0) {
            throw new TypeError("Operator count must be a non-negative integer.");
        }
        this.#operatorCount = operatorCount;
        this.#usesRemaining = Array(operatorCount).fill(0);
    ...
    getStationShops(stationId){
        return this.#stations[stationId].shops;
    }
    ...
    setConnectionStateChangeHandler(handler) {
        if (handler !== null && typeof handler !== "function") {
            throw new TypeError("Connection state change handler must be a function or null.");
        }
    ...
    setUsesRemaining(usesRemaining) {
        if (!Array.isArray(usesRemaining)
            || usesRemaining.length !== this.#operatorCount
            || usesRemaining.some(uses =>
                !Number.isInteger(uses) || uses < 0
            )) {
            throw new TypeError(
                "Uses remaining must contain one non-negative integer per operator."
            );
        }
        this.#usesRemaining = [...usesRemaining];
        localStorage.setItem(
            "_usesremaining",
            JSON.stringify(this.#usesRemaining)
        );
        if (this.#connectionStateChangeHandler !== null) {
    ...
    setEnergy(newEnergy) {
        if (!Number.isFinite(newEnergy)) {
            throw new TypeError("Energy must be a finite number.");
        }

        this.#energy = Math.max(0, Math.min(newEnergy, this.#maxEnergy));
        localStorage.setItem("_energy", String(this.#energy));
    }

    #readEnergy() {
        const stored = Number.parseFloat(localStorage.getItem("_energy"), 10);
        return Number.isNaN(stored) ? 5000 : stored;
    }

    #readEnergySnapshot() {
        const stored = JSON.parse(localStorage.getItem("_energySnapshot"));
        return stored == null ? {
            positionAtIdleStart: null, 
            energyRate: constants.ENERGY_RESTORATION_IDLE,
            lastEnergy: 5000,
            idleStartTime: null,
            savedAt: clock.getCurrentTimeInMilliseconds(this.#timeTravelled)} : stored;
    }

    setEnergySnapshotObject(energySnapshot){
        this.#energySnapshot = energySnapshot;

        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    }

    setEnergySnapshot(positionAtIdleStart, energyRate, lastEnergy, idleStartTime, savedAt){
        this.setEnergySnapshotPosition(positionAtIdleStart);
        this.setEnergySnapshotRate(energyRate);
        this.setEnergySnapshotLastEnergy(lastEnergy);
        this.setEnergySnapshotIdleStartTime(idleStartTime);
        this.setEnergySnapshotSavedAt(savedAt);
    }

    setEnergySnapshotPosition(positionAtIdleStart){
        this.#energySnapshot.positionAtIdleStart = positionAtIdleStart;
        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    ...
    setEnergySnapshotRate(energyRate){
        this.#energySnapshot.energyRate = energyRate;
        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    ...
    setEnergySnapshotLastEnergy(lastEnergy){
        this.#energySnapshot.lastEnergy = lastEnergy;
        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    ...
    setEnergySnapshotIdleStartTime(idleStartTime){
        this.#energySnapshot.idleStartTime = idleStartTime;
        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    ...
    setEnergySnapshotSavedAt(savedAt){
        this.#energySnapshot.savedAt = savedAt;
        localStorage.setItem(
            "_energySnapshot",
            JSON.stringify(this.#energySnapshot)
        );
    ...
    setMaxEnergy(newMaxEnergy) {
        if (!Number.isFinite(newMaxEnergy) || newMaxEnergy <= 0) {
            throw new TypeError("Maximum energy must be a positive finite number.");
        }

        this.#maxEnergy = newMaxEnergy;
        localStorage.setItem("_maxEnergy", String(this.#maxEnergy));
    ...
    #readMaxEnergy() {
        const stored = parseInt(localStorage.getItem("_maxEnergy"), 10);
        return Number.isNaN(stored) ? 5000 : stored;
    }
    ...
    setSelectedOperator(selectedOperator) {
        if (!Number.isInteger(selectedOperator)
            || selectedOperator < 0
            || selectedOperator > this.#operatorCount) {
            throw new TypeError(
                "Selected operator must be 0 for Wi-Fi or a configured operator number."
            );
        }
        this.#selectedOperator = selectedOperator;
        localStorage.setItem(
            "_selectedoperator",
            String(this.#selectedOperator)
        );
        if (this.#connectionStateChangeHandler !== null) {
            this.#connectionStateChangeHandler();
        }
    ...
    getOpenedOperatorSelection() {
        return this.#openedOperatorSelection;
    }

    setOpenedOperatorSelection(opened) {
        if (typeof opened !== "boolean") {
            throw new TypeError("Opened operator selection must be a boolean.");
        }
        this.#openedOperatorSelection = opened;
    }


    compareTrains(train1, train2){
        if (train1?.transporttype !== constants.TRANSPORT_TYPE.TRAIN || train2 === null){
            return false;
        }
    ...
    setCurrentPosition(position) {
        ...
        if (positionChanged) {
            ...
            const isBoardingTrain = nextPosition?.transporttype === constants.TRANSPORT_TYPE.TRAIN;
            if (!isBoardingTrain) {
    ...
    setAutoBoardSelection(selection) {
        ...
        const normalized = {
            ...
            stopIndex: selection.stopIndex == null
                ? null
                : Number(selection.stopIndex)
        };
        if (!Number.isInteger(normalized.lineID)
            || !Number.isInteger(normalized.tripID)
            || !Number.isInteger(normalized.day)
            || (normalized.stopIndex !== null
                && !Number.isInteger(normalized.stopIndex))) {
    ...
    purchaseAutoTravel({
        ...
        const normalizedSelection = {
            ...
            stopIndex: autoBoardSelection?.stopIndex == null
                ? null
                : Number(autoBoardSelection.stopIndex)
        };
        ...
        if (!Number.isInteger(normalizedSelection.lineID)
            ...
            || (normalizedSelection.stopIndex !== null
                && !Number.isInteger(normalizedSelection.stopIndex))
    ...
    #load() {
        ...
        this.#usesRemaining = this.#readUsesRemaining();
        this.#selectedOperator = this.#readSelectedOperator();
        ...
        this.#energy = this.#readEnergy();
        this.#maxEnergy = this.#readMaxEnergy();
        this.#energySnapshot = this.#readEnergySnapshot();
        ...
        position.hasConnection = false;
        delete position.iswifi;
        if (!("transporttype" in position)) position.transporttype = constants.TRANSPORT_TYPE.STATION;
        ...
        const hasCoordinateOrigin = (
            position.transporttype === constants.TRANSPORT_TYPE.WALKING
        ...
        const hasWalkingGoal = position.transporttype === constants.TRANSPORT_TYPE.WALKING
    ...
    #readUsesRemaining() {
        const stored = this.#readArray("_usesremaining");
        return Array.from(
            { length: this.#operatorCount },
            (_, index) => Number.isInteger(stored[index])
                && stored[index] >= 0
                ? stored[index]
                : 0
        );
    ...
    #readSelectedOperator() {
        const stored = Number.parseInt(
            localStorage.getItem("_selectedoperator"),
            10
        );
        return Number.isInteger(stored)
            && stored >= 0
            && stored <= this.#operatorCount
            ? stored
            : 0;
    }
