...
import * as districtData from "../generated/district-borders.js";
    ...
    function getProjection() {
        const allPoints = districtData.districtBorders.flatMap(district => district.borderVertices)
    ...
    function renderReachableStations() {
        const selectedStationId = Number(runtime.getStationSectionId());
        if (selectedStationId === renderedStationId) return;
        renderedStationId = selectedStationId;
        stationLayer.replaceChildren();

        const selectedStation = data.timetable.stations[selectedStationId];
        ...
        reachableStationIds.forEach(stationId => {
            appendStationMarker(data.timetable.stations[stationId], "map-reachable-station", 2.5);
        });
    ...
    function initialize() {
        ...
        districtData.districtBorders.forEach(district => {
    ...
    function render() {
        ...
        if (selectedDistrict === null && districtData.districtBorders.length > 0) {
            showDistrict(districtData.districtBorders[0]);
        }
    }
