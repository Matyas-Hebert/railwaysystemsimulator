...
async function checktimetable(){
    ...
    let stationimportancedata = {};
    timetable.lines.forEach(line => {
        let length = line.stops.length;
        let interval = line.interval/(60/2);
        let type = line.type;
        let weight = 1;
        if (type == -1){
            weight = 0;
        ...
        let typestring = getTypeString(type);
        for(let i=0; i<length; i++){
            let station = line.stops[i].sid;
            if (stationimportancedata[station] == undefined){
                stationimportancedata[station] = {
            ...
            stationimportancedata[station].importance += (60/interval)*weight;
            stationimportancedata[station].trainsperhour[typestring] += (60/interval);
            if (i != 0 || i != length-1){
                stationimportancedata[station].trainsperhour[typestring] += (60/interval);
            }
        }
    ...
    let sortedimportancedata = Object.values(stationimportancedata).sort((a, b) => b.importance - a.importance);

    for(let pos=1; pos<=4000; pos++){
        let data = sortedimportancedata[pos-1];
        console.log(String(pos)+". "+data.name, Math.round(data.importance*100)/100, 
            "Ps", Math.round(data.trainsperhour["Ps"]*10)/10,
            "Os", Math.round(data.trainsperhour["Os"]*10)/10,
            "Sp", Math.round(data.trainsperhour["Sp"]*10)/10,
            "R ", Math.round(data.trainsperhour["R "]*10)/10,
            "Sh", Math.round(data.trainsperhour["Sh"]*10)/10,
            "EC", Math.round(data.trainsperhour["EC"]*10)/10
        );
    };

    sortedimportancedata = Object.values(stationimportancedata).sort((a, b) => b.trainsperhour["Os"] - a.trainsperhour["Os"]);
    ...
    sortedimportancedata = Object.values(stationimportancedata).sort((a, b) => b.trainsperhour["Sp"] - a.trainsperhour["Sp"]);
    ...
    sortedimportancedata = Object.values(stationimportancedata).sort((a, b) => b.trainsperhour["R "] - a.trainsperhour["R "]);
    ...
    sortedimportancedata = Object.values(stationimportancedata).sort((a, b) => b.trainsperhour["EC"] - a.trainsperhour["EC"]);
